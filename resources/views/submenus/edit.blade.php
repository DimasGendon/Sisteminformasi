...
<div class="container">
    <form action="{{ route('submenus.update', $submenu->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" id="name" class="form-control" value="{{ $submenu->name }}">
        </div>
        <div class="form-group">
            <label for="photo">Foto :</label>
            <input type="file" name="photo" id="photo" class="form-control">
            @if($submenu->foto)
                <p>Foto saat ini: <img src="{{ asset('storage/' . $submenu->foto) }}" alt="Foto" width="50"></p>
            @endif
        </div>
        <div class="form-group">
            <label for="description">Deskirpsi :</label>
            <!-- Textarea untuk CKEditor -->
            <textarea name="description" id="editor" class="form-control">{{ $submenu->description }}</textarea>
        </div>
